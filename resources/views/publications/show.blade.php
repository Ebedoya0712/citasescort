<x-main-layout>
    ...
    <x-slot:title>
        {{ $publication->title }} - {{ $escort->name }} en {{ $publication->city }}
        </x-slot>
            ...
            <div class="max-w-7xl mx-auto px-4 lg:px-8">
                <div class="flex flex-col md:flex-row gap-8 items-center md:items-start">
                    ...
                    <div class="relative w-48 h-48 md:w-64 md:h-64 flex-shrink-0"
                         x-data="{ 
                             hasStories: {{ $escort->stories->count() > 0 ? 'true' : 'false' }},
                             unseenStories: false,
                             checkStories() {
                                 if (!this.hasStories) return;
                                 let seen = JSON.parse(localStorage.getItem('citas_seen_stories') || '[]');
                                 let escortStories = {{ $escort->stories->pluck('id')->toJson() }};
                                 this.unseenStories = escortStories.some(id => !seen.includes(id));
                             }
                         }"
                         x-init="checkStories()"
                         @story-seen.window="checkStories()">
                        <div @click="hasStories && $dispatch('open-story-viewer')"
                            ...
                            <div class="relative w-full h-full rounded-full overflow-hidden bg-black">
                                ...
                                @elseif($publication->photos && count($publication->photos) > 0)
                                    @php
                                        $photo = $publication->photos[0];
                                        $src = Str::startsWith($photo, ['http://', 'https://']) ? $photo : Storage::url($photo);
                                    @endphp
                                    <img src="{{ $src }}" alt="{{ $publication->title }}" class="w-full h-full object-cover">
                        ...
                        @if($escort->isVerified())
                            <div class="absolute -bottom-2 -right-2 z-10 w-16 h-16 md:w-20 md:h-20 bg-black/80 rounded-full flex items-center justify-center border-2 border-red-600/80 shadow-lg shadow-black/80 select-none overflow-hidden">
                                <!-- Rotating Circular Text -->
                                <svg class="absolute w-full h-full" style="animation: spin-slow 15s linear infinite;" viewBox="0 0 100 100">
                                    <defs>
                                ...
                                <div class="relative w-7 h-7 md:w-9 md:h-9 flex items-center justify-center">
                                    ...
                                    <svg class="absolute w-3.5 h-3.5 md:w-4.5 h-4.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="stroke-width: 4;">
                    ...
                    <div class="flex-1 text-center md:text-left space-y-4">
                        ...
                        <!-- Detalles Personales Bar (Cintura, Estatura, Caderas, Busto) -->
                        <div class="bg-gradient-to-r from-red-600 to-red-700 text-white rounded-3xl p-6 flex flex-wrap justify-around items-center gap-6 md:gap-4 shadow-lg shadow-red-900/20 max-w-2xl my-6 select-none">
                            <!-- Cintura -->
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-full bg-white/10 flex items-center justify-center flex-shrink-0 text-white">
                                    <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                        <path d="M5 3c2 2 2 6 0 10s-2 6 0 8" />
                                        <path d="M19 3c-2 2-2 6 0 10s2 6 0 8" />
                                        <path d="M11 9h2" />
                                        <path d="M10 13h4" />
                                ...
                                <div>
                                    <div class="text-xl font-bold tracking-tight leading-none">{{ $publication->waist ?? $escort->waist ?? '---' }}</div>
                                    <div class="text-xs text-red-100 font-medium mt-1">Cintura</div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="text-xl font-bold tracking-tight leading-none">
                                        @php
                                            $heightVal = $publication->height ?? $escort->height;
                                            if ($heightVal && is_numeric($heightVal) && $heightVal > 3) {
                                                $heightVal = number_format($heightVal / 100, 2) . 'm';
                                            } elseif ($heightVal && is_numeric($heightVal)) {
                                                $heightVal = number_format($heightVal, 2) . 'm';
                                            }
                                        @endphp
                                        {{ $heightVal ?? '---' }}
                                    </div>
                                    <div class="text-xs text-red-100 font-medium mt-1">Estatura</div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="text-xl font-bold tracking-tight leading-none">{{ $publication->hips ?? $escort->hips ?? '---' }}</div>
                                    <div class="text-xs text-red-100 font-medium mt-1">Caderas</div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-full bg-white/10 flex items-center justify-center flex-shrink-0 text-white">
                                    <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                        <path d="M6 18c-3 0-4-3-4-5s2-5 4-5 4 3 4 5-1 5-4 5z" />
                                        <path d="M18 18c-3 0-4-3-4-5s2-5 4-5 4 3 4 5-1 5-4 5z" />
                                        <path d="M10 13h4" />
                                    </svg>
                                </div>
                                <div>
                                    <div class="text-xl font-bold tracking-tight leading-none">{{ $publication->bust ?? $escort->bust ?? '---' }}</div>
                                    <div class="text-xs text-red-100 font-medium mt-1">Busto</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="text-gray-400 text-sm leading-relaxed max-w-2xl">
                            @php
                                $descriptionLength = strlen($publication->description ?? '');
                                $isLongDescription = $descriptionLength > 200;
                                $shortDescription = $isLongDescription ? substr($publication->description, 0, 200) . '...' : $publication->description;
                            @endphp

                            @if($isLongDescription)
                                <span id="short-description">{{ $shortDescription }}</span>
                                <span id="full-description" class="hidden">{{ $publication->description }}</span>
                        ...
                        <div class="flex flex-wrap gap-4 pt-4">
                            ...
                            @if($publication->telegram || $escort->telegram)
                                @php
                                    $telegramUser = $publication->telegram ?? $escort->telegram;
                                    $telegramUser = ltrim($telegramUser, '@');
                                @endphp
                                <a href="https://t.me/{{ $telegramUser }}" target="_blank"
                                    class="bg-sky-500 hover:bg-sky-400 text-white py-2.5 px-6 rounded-lg font-bold flex items-center gap-2 transition-all min-w-[160px] justify-center shadow-lg shadow-sky-900/20">
                                    <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                                        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-1-.65-.35-1 .22-1.62.15-.16 2.72-2.49 2.77-2.7.01-.03.01-.14-.06-.2-.07-.06-.18-.04-.25-.02-.11.02-1.83 1.16-5.16 3.42-.49.34-.93.5-1.33.49-.44-.01-1.29-.25-1.92-.45-.77-.25-1.38-.39-1.33-.82.03-.22.33-.45.92-.69 3.59-1.56 5.98-2.59 7.18-3.08 3.42-1.4 4.13-1.64 4.59-1.65.1 0 .33.02.48.15.13.11.16.27.18.38.02.13.02.27.01.4z"/>
                                    </svg>
                                    Telegram
                                </a>
                            @endif
            ...
            <div class="max-w-7xl mx-auto px-4 lg:px-8 py-8">
                ...
                @php
                    ...
                    if (!empty($publication->photos)) {
                        foreach ($publication->photos as $photo) {
                    ...
                    $orderedPhotos = array_merge($photosOnly, $videosOnly);

                    $lightboxMedia = [];
                    if (!empty($orderedPhotos)) {
                        $lightboxMedia = collect($orderedPhotos)->map(function ($photo) {
                            $src = Str::startsWith($photo, ['http://', 'https://']) ? $photo : Storage::url($photo);
                            $extension = pathinfo($photo, PATHINFO_EXTENSION);
                            $isVideo = in_array(strtolower($extension), ['mp4', 'mov', 'avi', 'webm']);
                            return [
                ...
                <div x-data='{
                    ...
                    <!-- Gallery Grid Separated -->
                    <div class="space-y-10">
                        <!-- Section: Mis Fotos -->
                        @if(!empty($photosOnly))
                            <div>
                                <h2 class="text-2xl font-bold text-white mb-6 flex items-center gap-3">
                                    <div style="background-color: #dc2626; width: 36px; height: 36px; border-radius: 9999px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; color: white; box-shadow: 0 4px 6px -1px rgba(220, 38, 38, 0.2);">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    @foreach($photosOnly as $index => $photo)
                                        @php
                                            $src = Str::startsWith($photo, ['http://', 'https://']) ? $photo : Storage::url($photo);
                                        @endphp
                                        <div @click="openLightbox({{ $index }})"
                                             class="aspect-[3/4] bg-zinc-900 rounded-lg overflow-hidden group relative cursor-pointer shadow-lg shadow-black/40 border border-zinc-800/50">
                        ...
                        @if(!empty($videosOnly))
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    @foreach($videosOnly as $index => $video)
                                        @php
                                        ...
                                        <div @click="openLightbox({{ $actualIndex }})"
                                             ...
                                             <video src="{{ $src }}#t=0.1" loop muted autoplay playsinline preload="auto"
                        ...
                        @if(empty($photosOnly) && empty($videosOnly))
                            <div class="text-center py-10 text-gray-500">No hay fotos ni videos disponibles.</div>
                        @endif
                    </div>
                ...
                <div class="mt-12 border-y border-zinc-700 bg-black">
                    <div class="grid grid-cols-1 md:grid-cols-5 divide-y md:divide-y-0 md:divide-x divide-zinc-700">
                        ...
                        <div class="p-6">
                            ...
                            @php
                                $loc = array_filter([
                                    $publication->city ?? $escort->city ?? '',
                                    $publication->province ?? $escort->province ?? '',
                                    $publication->district ?? $escort->district ?? ''
                                ]);
                            @endphp
                        ...
                        <div class="p-6">
                            ...
                            <div class="text-gray-300 text-sm space-y-1">
                                ...
                                <div>1 hora - S/{{ number_format($publication->price, 0) }}</div> @endif
                                @if($escort->cost_30m)
                        ...
                        <div class="p-6">
                            ...
                            <div class="text-gray-300 text-sm">
                                @if(!empty($publication->attends_in))
                                    {{ implode(', ', $publication->attends_in) }}
                                @elseif(!empty($escort->attends_in))
                        ...
                        <div class="p-6">
                            ...
                            <div class="text-gray-300 text-sm">
                                @if(!empty($publication->attends_to))
                                    {{ implode(', ', $publication->attends_to) }}
                                @elseif(!empty($escort->attends_to))
